...
n_sub_hrdt <- 60
n_trials_hrdt <- 40
delta_bpm <- rep(seq(-40, 40, 1),n_trials_hrdt) # might need to change range based on what's plausible

```

```{r}
sim_data_sigrid <- expand.grid(
  subject = factor(1:n_sub_hrdt),
  trial   = 1:n_trials_hrdt
)

sim_data_sigrid$stimulus <- sample(delta_bpm, nrow(sim_data_sigrid), replace = TRUE) # sampling with replacement, independent samples
...
sim_data_sigrid$p_faster <- psychometric_f(
  x     = sim_data_sigrid$stimulus,
  alpha = alpha,
  beta  = beta
)

# generate responses
sim_data_sigrid$response <- rbinom(
  n = nrow(sim_data_sigrid),
...
$$
where F(x) is one's psychometric function.
...
# Conceptual definition of Jesper's more advanced model (probit regression) with lapse rate
...
# new 
sim_data_sigrid = data.frame(stimulus = rnorm(N,0,10), subject = 1) %>% mutate(trial = 1:n())
...
sim_data_lapse$response <- rbinom(
  n = nrow(sim_data_lapse),
  size = 1,
  prob = lambda + (1-2*lambda)*inv_logit((sim_data_lapse$stimulus - alpha)/beta)
)
...
sim_data = data.frame(stimulus = rnorm(N,0,10), subject = 1) %>% mutate(trial = 1:n())
...
lav ny model, der sampler stimulus som jesper foreslår, ud fra deltageres threshold (hvordan gør jeg det) og med 2x beta SD
